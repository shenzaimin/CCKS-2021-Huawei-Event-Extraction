...
def parse_arguments_t(parser):
    ...
    parser.add_argument('--ensemble', type=int, default=0, help="how many models for model ensemble")
    parser.add_argument('--lambdalr', type=float, default=0.001, help="how many models for model ensemble")
...
def pretrain_main():
    ...
    optimizer = AdamW(optimizer_grouped_parameters, lr=conf.learning_rate, eps=1e-8)
...
def main():
    ...
    if opt.ensemble == 0:
        ...
        optimizer = AdamW(optimizer_grouped_parameters, lr=conf.learning_rate, eps=1e-8)
        scheduler = LambdaLR(optimizer, lr_lambda=lambda epoch: 1 / (1 + 0.001 * epoch))
        #scheduler = LambdaLR(optimizer, lr_lambda=lambda epoch: 1 - (epoch / conf.num_epochs))
        ...
        for ensemble_id in range(opt.ensemble):
            set_seed(opt, conf.seed + ensemble_id)
            temp_model_folder = os.path.join(conf.model_folder, 'ensemble/' + str(ensemble_id))
            if not os.path.exists(temp_model_folder):
                os.makedirs(temp_model_folder)
            # set logger
            utils.set_logger(os.path.join(temp_model_folder, 'train.log'))
            ...
            logging.info("Loading the datasets...")
            trains_add_devs = reader.read_txt(conf.train_file, conf.train_num, opt.type)
            #trains = trains_add_devs[:int(ensemble_id*len(trains_add_devs)/opt.ensemble)] + trains_add_devs[int((ensemble_id+1)*len(trains_add_devs)/opt.ensemble):]
            trains = trains_add_devs
            devs = trains_add_devs[int(ensemble_id*len(trains_add_devs)/opt.ensemble):int((ensemble_id+1)*len(trains_add_devs)/opt.ensemble)]
            print('【trains: ' + str(len(trains)) + ' devs: ' + str(len(devs)) + '】')
            # import numpy as np
            # import pandas as pd
            # id_list = np.asarray([t.id for t in trains])
            ...
            # Initialize bert model
            logging.info("Initialized from pre-trained Model")
            model_folder = temp_model_folder
            model_name = model_folder + "/final_bert_crf"
            logging.info("[Training Info] The model will be saved to: %s" % (model_folder))
            config_name = model_folder + "/config.conf"
            # load config for bertCRF
            ...
            scheduler = LambdaLR(optimizer, lr_lambda=lambda epoch: 1 / (1 + opt.lambdalr * epoch))
            #scheduler = LambdaLR(optimizer, lr_lambda=lambda epoch: 1 - (epoch / conf.num_epochs))
            model.train()
            epoch = conf.num_epochs
            best_dev_f1 = -1
            for i in range(1, epoch + 1):
                ...
                with torch.no_grad():
                    # metric is [precision, recall, f_score]
                    dev_metrics = evaluate_model(conf, model, dev_batches, "dev", devs)
                    if True or dev_metrics[2] > best_dev_f1:  # save the best model
                        logging.info("saving the best model...")
                        best_dev_f1 = dev_metrics[2]
...
def main_predict():
    ...
    # 读取分类结果
    import pandas as pd
    cls_out = pd.read_csv('CCKS-Cls/test_output/cls_out_single.csv')
    cls_dict = dict()
    ...
    for suffix in ['sg', 'pj', 'db', 'zb', 'qsht']:
        ...
        cfig.label_size = conf.label_size
        cfig.idx2labels = conf.idx2labels
        temp_model_folder = conf.model_folder + "_" + suffix + '_1_1'
        tests_list = []
        ensemble_lst = []
        ensemble_cnt = []
        for ensemble_id in range(10):
            tests_temp = copy.deepcopy(tests)
            if os.path.exists(os.path.join(temp_model_folder, 'ensemble/' + str(ensemble_id))):
                model_folder = os.path.join(temp_model_folder, 'ensemble/' + str(ensemble_id))
            else:
                break
            model_name = model_folder + "/final_bert_crf"
            # model = BertCRF.from_pretrained(conf.bert_model_dir, config=cfig)
            ...
            '''
            for idx in range(len(tests_temp)):
                prediction = tests_temp[idx].output_ids
                prediction = [cfig.idx2labels[l] for l in prediction]
                # 测试一下标答labels的抽取
                # prediction = tests[idx].output
                tests_temp[idx].prediction = prediction

            for idx in range(len(tests_temp)):
                qids = tests_temp[idx].id
                data_type = type_map[suffix]
                start = -1
                for i in range(len(tests_temp[idx].prediction)):
                    if tests_temp[idx].prediction[i].startswith("B-") and start == -1:
                        start = i
                        # 找出单字实体(仅针对NUM类别)
                        ...
                        #                                                           "end_position": i}]
                        #         start = -1
                    if tests_temp[idx].prediction[i].startswith("E-") and start != -1:
                        # if i != len(tests[idx].prediction) - 1 and predict_dict['query'][i+1] == '之':  # 修正模型对于TV类别预测的不恰当分割
                        #     continue
                        # else:
                        if tests_temp[idx].prediction[i][2:] == tests_temp[idx].prediction[start][
                                                           2:]:  # START 和 END 的类别必须保持一致，否则不能算实体，放弃抽取
                            end = i
                            value = tests_temp[idx].content[start:end+1]
                            role = tests_temp[idx].prediction[i][2+len(suffix):]
                            sample = {"id": qids, "events": [{"type": data_type,
                                                              "mentions": [
                                                                  {"word": value, "span": [start, end+1],
                                                                   "role": role
                                                                   }]}]}
                            if sample not in ensemble_lst:
                                ensemble_lst.append(sample)
                                ensemble_cnt.append(0)
                            #print(ensemble_cnt[ensemble_lst.index(sample)])
                            ensemble_cnt[ensemble_lst.index(sample)] += 1
                            #print(ensemble_cnt[ensemble_lst.index(sample)])
                            start = -1
                        else:
                            start = -1
        ...
        '''
        if ensemble_lst != []:
            for i in range(len(ensemble_cnt)):
                #print(ensemble_lst[i])
                #print(ensemble_cnt[i])
                if ensemble_cnt[i] >= 5:
                    lst.append(ensemble_lst[i])
            continue
        '''
        if ensemble_id != 0:
            ...
            for i in range(len(tests[idx].prediction)):
                ...
                if tests[idx].prediction[i].startswith("E-") and start != -1:
                    ...
                    if tests[idx].prediction[i][2:] == tests[idx].prediction[start][
                        ...
                        start = -1
    sub_data =  open('result_tmp.json', 'w+', encoding='utf-8')
